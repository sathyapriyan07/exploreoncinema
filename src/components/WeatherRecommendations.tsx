...
import { MOOD_ICONS, type LucideIcon } from './ui/Icon';
import {
    ...
    <section className="pt-2 pb-6">
      ...
      <div className="px-6 md:px-12 mb-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {mood && (() => {
                  ...
                  return (
                    <span className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-yellow-500/15 text-yellow-400 text-xs font-bold border border-yellow-500/20">
          ...
          <div className="hidden md:flex gap-1 shrink-0">
            <button onClick={() => scroll('left')} className="h-7 w-7 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors" aria-label="Scroll left">
              <ChevronLeft className="h-4 w-4" />
            </button>
            <button onClick={() => scroll('right')} className="h-7 w-7 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition-colors" aria-label="Scroll right">
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide mt-4 pb-1">
          {ALL_MOODS.map((m) => {
            ...
            const isActive = mood === m && manualMood === m;
            return (
              <button
                key={m}
      ...
      <div className="px-6 md:px-12 mb-4">
        <h2 className="text-xl font-bold tracking-tight flex items-center gap-2">
          {mood && (() => {
            const MoodIcon = MOOD_ICONS[mood];
            return <MoodIcon className="h-5 w-5 text-yellow-400" />;
          })()}
          {mood ? `Picks for Your Mood` : 'Recommended for You'}
        </h2>
      </div>
