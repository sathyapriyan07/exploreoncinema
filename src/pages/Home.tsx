...
function MovieRow({ title, data, loading, type }: {
  ...
  const expand = (id: number, t: 'movie' | 'tv') => setExpanded(e => e?.id === id ? null : { id, type: t });
        ...
        {loading
          ...
          : data?.results?.slice(0, 20).map((item: any) => (
              <div key={item.id} className="w-[130px] md:w-[160px] shrink-0">
                <ContentCard item={item} type={item.media_type || type} onExpand={() => expand(item.id, item.media_type || type)} />
              </div>
            ))}
      </div>
